...
func CreateConnectionContent(mysqlIP, mysqlPort, proxyPort string) *tview.Flex {
	table := tview.NewTable().
		SetBorders(false)

	// MySQL - Currently connections
	table.SetCell(0, 0, tview.NewTableCell(fmt.Sprintf("[default]MySQL: [yellow]%s:%s", mysqlIP, mysqlPort)).
	...
	table.SetCell(1, 0, tview.NewTableCell(fmt.Sprintf("[default]Proxy: [green]127.0.0.1:%s", proxyPort)).
		SetTextColor(tview.Styles.PrimaryTextColor).
		SetAlign(tview.AlignLeft).
		SetExpansion(1))
